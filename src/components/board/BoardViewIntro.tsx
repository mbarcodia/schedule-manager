...
const INTROS: Record<BoardViewId, { title: string; body: string }> = {
  ...
  todos: {
    ...
    body:
      "Lists you name yourself, holding anything from a one-line errand to a talk you have to prepare for. An item starts as plain text and stays that way unless you ask for more: open it and you can set when it happens, ask to be reminded a week and a day beforehand, and book hours on your calendar for it. A booking has a start and a finish-by, which is also how you book preparation: two hours finished by the morning of the day itself, for something happening that afternoon. You can add any of that later, so jotting something down now and deciding in three weeks that it needs real time is the normal path, not a redo. Ticking an item off also clears whatever hours it had booked, so your calendar never holds time for work you\u2019ve already finished. Under each item is a small grey line \u2014 click it to open those settings, or to see at a glance what an item already has. Each list can also watch itself: choose a rhythm and anything still unticked when the week, month or year ends arrives as one notification.",
  },
  lists: {
