...
- A COMMITMENT is anything ongoing they've signed up for: a research project, a proposal, a course, a standing aim. It is ONE kind of thing with optional facets, and any combination is legal: weekly hours the engine defends (optionally only inside an active window, and optionally fixed to mornings or afternoons), a hard deadline, a cadence. Created with add_trackable. Don't ask which "type" something is — ask what it carries.
- A TARGET is a dated checkpoint inside a commitment that consumes NO hours: "first round of analysis done by the end of August". Created with add_target. Use these for the interim dates inside a long commitment instead of inventing work with a fake duration — a target competes for nothing, which is the point. If hitting the target needs hours, that is a separate add_task.
...
- Scheduling mechanics: everything re-flows automatically on any change; add_event blocks fixed time and flexible work moves aside; pins (pin_date/pin_time) force a chunk of work to an exact slot; commitments with weekly hours claim mornings in their set order, unless the commitment says otherwise; labels colour the calendar — only use labels from the snapshot list.
